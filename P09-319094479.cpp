...
int main()
{
	...
	Fuente = Texture("Textures/FuenteMario.jpg");
	Fuente.LoadTextureA();
	...
	Puerta1 = Model();
	Puerta1.LoadModel("Models/Reja1.obj");
	Puerta2 = Model();
	Puerta2.LoadModel("Models/Reja2.obj");
	Dragon = Model();
	...
	while (!mainWindow.getShouldClose())
	{
		...
		aleteo = 30.0f * sin(glm::radians(angulovaria * 3.0f));
		movCabeza += 0.03f * deltaTime;
		//Dragon
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(0.0f, 5.0f+sin(glm::radians(angulovaria)), 6.0f));
		...
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		AlaI.RenderModel();
		//Cabeza1(roja, espiral de Arquímedes)
		a = 0.5f * movCabeza;
		b = a * cos(movCabeza);
		c = a * sin(movCabeza);
		model = modelaux;
		model = glm::translate(model, glm::vec3(b, c, 0.0f));
		Material_brillante.UseMaterial(uniformSpecularIntensity, uniformShininess);
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		Cabeza1.RenderModel();
		//Cabeza2(azul, movimiento senoidal)
		model = modelaux;
		model = glm::translate(model, glm::vec3(0.0f, sin(movCabeza) * 2.0f, 0.0f));
		Material_brillante.UseMaterial(uniformSpecularIntensity, uniformShininess);
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		Cabeza2.RenderModel();
		//Cabeza3(verde, lemniscata de Bernoulli)
		d = 2.0f;
		e = 1 + pow(sin(movCabeza), 2);
		b = (d * cos(movCabeza)) / e;
		c = (d * cos(movCabeza) * sin(movCabeza)) / e;
		model = modelaux;
		model = glm::translate(model, glm::vec3(b, c, 0.0f));
		Material_brillante.UseMaterial(uniformSpecularIntensity, uniformShininess);
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		Cabeza3.RenderModel();
		//Cabeza4(blanca, movimiento circular)
		model = modelaux;
		model = glm::translate(model, glm::vec3(1.0f * cos(movCabeza * 2), 1.0f * sin(movCabeza * 2), 0.0f));
		Material_brillante.UseMaterial(uniformSpecularIntensity, uniformShininess);
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		Cabeza4.RenderModel();
		//Cabeza5(cafe, rosa polar)
		i = 3.0f;
		j = 4.0f;
		k = i * cos(j * movCabeza);
		m = k * cos(movCabeza);
		n = k * sin(movCabeza);
		model = modelaux;
		model = glm::translate(model, glm::vec3(m, n, 0.0f));
		Material_brillante.UseMaterial(uniformSpecularIntensity, uniformShininess);
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		Cabeza5.RenderModel();
		...
		*/
		...
		if (tiempoTextura >= intervaloTextura) {
			textura = !textura;
			tiempoTextura = 0.0f;
		}
		if (textura) {
